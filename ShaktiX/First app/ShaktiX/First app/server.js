...
// Helper function to add timeout to blockchain calls
function withTimeout(promise, timeoutMs = 30000) {
  const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Blockchain call timed out')), timeoutMs));
  return Promise.race([promise, timeoutPromise]);
}

// Initialize Polygon Blockchain Connection
...
try {
  ...
  if (POLYGON_RPC_URL && PRIVATE_KEY && CONTRACT_ADDRESS) {
    ...
    signer = new ethers.Wallet(PRIVATE_KEY, polygonProvider);

    // Contract ABI (simplified for our functions)
    const contractABI = [
      "function storeEvidence(bytes32 evidenceHash, string memory filename, string memory metadata) public returns (uint256)",
      "function getEvidenceByHash(bytes32 evidenceHash) public view returns (uint256, bytes32, uint256, string memory, string memory, address, bool)",
      "function getEvidenceById(uint256 id) public view returns (uint256, bytes32, uint256, string memory, string memory, address, bool)",
      "function verifyEvidence(bytes32 storedHash, bytes32 providedHash) public returns (bool)",
      "function getTotalEvidence() public view returns (uint256)",
      "function accessEvidence(bytes32 oldHash) public returns (bytes32)"
    ];

    evidenceContract = new ethers.Contract(CONTRACT_ADDRESS, contractABI, signer);

    console.log('✅ Polygon blockchain connection initialized successfully');
    console.log('📍 Contract Address:', CONTRACT_ADDRESS);
    console.log('👤 Signer Address:', signer.address);
  } else {
    console.log('⚠️ Polygon blockchain not configured - set POLYGON_RPC_URL, PRIVATE_KEY, and CONTRACT_ADDRESS for blockchain storage');
  }
} catch (error) {
  console.error('❌ Failed to initialize Polygon blockchain:', error.message);
}
...
app.post('/api/evidence/store', async (req, res) => {
  try {
    ...
    // Attempt to anchor on-chain if configured
    let onChainId = null;
    let onChainHash = null;
    let blockchainAnchor = `anchor-${Date.now()}`; // default simulated anchor

    try {
      if (evidenceContract) {
        const bytes32Hash = ('0x' + String(hash).replace(/^0x/, '')).toLowerCase();
        if (bytes32Hash.length !== 66) {
          throw new Error('Invalid SHA-256 hash length for bytes32');
        }

        console.log('🔗 Anchoring evidence on-chain...');
        // First, try a static call to ensure it will succeed (ethers v6)
        await evidenceContract.storeEvidence.staticCall(bytes32Hash, filename, JSON.stringify(metadata || {}));
        const tx = await evidenceContract.storeEvidence(bytes32Hash, filename, JSON.stringify(metadata || {}));
        const receipt = await tx.wait();
        console.log('✅ On-chain tx confirmed:', receipt.transactionHash);

        // Retrieve stored record by hash to get its on-chain ID and details
        const stored = await evidenceContract.getEvidenceByHash(bytes32Hash);
        // stored = (id, evidenceHash, timestamp, filename, metadata, owner, verified)
        const contractId = Number(stored[0] || stored.id || 0);
        if (contractId > 0) {
          onChainId = contractId;
          onChainHash = stored[1];
          blockchainAnchor = onChainHash;
        } else {
          console.warn('⚠️ Evidence not found on-chain after tx; keeping simulated anchor');
        }
      } else {
        console.log('⚠️ Blockchain not configured; using simulated anchor');
      }
    } catch (chainErr) {
      console.error('❌ Failed to anchor on-chain:', chainErr.message);
      // Continue with simulated anchor for resilience
    }

    // Create evidence record
    const evidenceRecord = {
      id: `evidence-${Date.now()}`,
      hash: hash, // original file SHA-256 (immutable content hash)
      timestamp: timestamp,
      filename: filename,
      fileSize: fileSize || 0,
      ...
      verified: true,
      blockchainAnchor: blockchainAnchor, // real on-chain hash if available
      onChainId: onChainId,
...
app.get('/api/evidence/:id', async (req, res) => {
  try {
    ...
    let evidenceRecord = evidenceStorage.get(id) || null;
    let source = 'file';

    // If we have a local record and blockchain is configured, rotate hash on access as per requirements
    if (evidenceRecord && evidenceContract && evidenceRecord.onChainHash) {
      try {
        console.log('🔗 Rotating on-chain hash due to access...');
        // Compute new hash via staticCall, then send the tx
        const newHash = await evidenceContract.accessEvidence.staticCall(evidenceRecord.onChainHash);
        const tx = await evidenceContract.accessEvidence(evidenceRecord.onChainHash);
        await tx.wait();
        console.log('✅ Access logged & hash rotated on-chain');

        // Update local mapping to point to new on-chain hash
        evidenceRecord.onChainHash = newHash;
        evidenceRecord.blockchainAnchor = newHash;
        saveEvidenceDatabase();
        source = 'blockchain';
      } catch (rotateErr) {
        console.error('❌ Failed to rotate hash on access:', rotateErr.message);
      }
    }

    // If still no record (e.g., not found locally), try to fetch from blockchain by numeric ID if provided
    if (!evidenceRecord && evidenceContract) {
      try {
        const numericId = parseInt(id, 10);
        if (!Number.isNaN(numericId)) {
          const [contractId, hashOnChain, ts, fname, metadataOnChain, owner, verified] = await evidenceContract.getEvidenceById(numericId);
          if (contractId > 0) {
            evidenceRecord = {
              id: id,
              hash: '', // unknown original SHA-256 without local mirror
              ...
              blockchainAnchor: hashOnChain,
              onChainId: Number(contractId),
              onChainHash: hashOnChain,
              status: 'preserved'
            };
            source = 'blockchain';
          }
        ...
        console.error('❌ Blockchain direct fetch failed:', chainFetchErr.message);
      }
    }
    ...
    console.log(`✅ Evidence found from ${source}:`, id);
    res.json({
      success: true,
      evidence: evidenceRecord,
      source: source,
...
app.post('/api/evidence/verify-hash', async (req, res) => {
  try {
    ...
    // First, try to validate against the blockchain directly if configured
    if (evidenceContract) {
      try {
        const bytes32Hash = ('0x' + String(hash).replace(/^0x/, '')).toLowerCase();
        if (bytes32Hash.length === 66) {
          const [contractId, evidenceHash, ts, fname, metadataOnChain, owner, verified] = await evidenceContract.getEvidenceByHash(bytes32Hash);
          if (Number(contractId) > 0) {
            return res.json({
              success: true,
              verification: {
                found: true,
                evidenceId: Number(contractId),
                filename: fname,
                originalTimestamp: new Date(Number(ts) * 1000).toISOString(),
                verificationTimestamp: new Date().toISOString(),
                integrity: 'INTACT',
                chainOfCustody: 'VERIFIED',
                blockchainStatus: 'ANCHORED',
                blockchainAnchor: evidenceHash
              },
              message: 'Evidence hash verified on-chain'
            });
          }
        }
      } catch (onChainVerifyErr) {
        console.warn('⚠️ On-chain hash verification failed:', onChainVerifyErr.message);
      }
    }
    ...
    if (matchingRecord) {
      return res.json({
        ...
        verification: {
          ...
          integrity: 'INTACT',
          chainOfCustody: 'VERIFIED',
          blockchainStatus: matchingRecord.onChainHash ? 'ANCHORED' : 'LOCAL_ONLY',
          blockchainAnchor: matchingRecord.blockchainAnchor
        },
        message: 'Evidence hash verified in local database'
      });
    }
    ...
    return res.json({
      ...
      message: 'Evidence hash not found'
    });
...
// Rotate on-chain hash on explicit access request and update local mapping
app.post('/api/evidence/access', async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: 'Evidence ID is required' });

    // Reload and fetch record
    loadEvidenceDatabase();
    const record = evidenceStorage.get(id);
    if (!record) return res.status(404).json({ error: 'Evidence not found' });
    if (!evidenceContract || !record.onChainHash) {
      return res.status(400).json({ error: 'Blockchain not configured or record not anchored on-chain' });
    }

    const newHash = await evidenceContract.callStatic.accessEvidence(record.onChainHash);
    const tx = await evidenceContract.accessEvidence(record.onChainHash);
    const receipt = await tx.wait();
    console.log('✅ Access logged via /api/evidence/access:', receipt.transactionHash);

    record.onChainHash = newHash;
    record.blockchainAnchor = newHash;
    evidenceStorage.set(id, record);
    saveEvidenceDatabase();

    res.json({
      success: true,
      evidence: record,
      message: 'Evidence access logged and hash rotated on-chain'
    });
  } catch (error) {
    console.error('❌ Evidence access rotation error:', error);
    res.status(500).json({ error: 'Failed to log access / rotate hash', details: error.message });
  }
});
